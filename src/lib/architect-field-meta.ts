  ...
  if (metaKey && CRITERION_META[metaKey]) {
    ...
    if (source === "openai") {
      return {
        ...
        calculation:
          "Evaluated by OpenAI from the business submission (title, description, problem, solution, metadata) and optional architect brief.",
      };
    }
    return base;
  }
...
export function getTelecomAreaMeta(
  ...
  return {
    ...
    calculation:
      source === "openai"
        ? "Relevance score assigned by OpenAI from use case text, category, and telecom context."
...
export function getQuestionMeta(
  ...
  return {
    ...
    calculation:
      source === "openai"
        ? "Generated by OpenAI from unmet readiness criteria and gaps in the business submission."
...
export function getDimensionMeta(
  ...
  if (source === "openai") {
    return {
      meaning: base.meaning,
      calculation:
        "Percentage of checklist criteria marked met by OpenAI after reading the full business submission.",
    };
  }
  return base;
}
